...
- 执行 `git add docs/ledgers/2026-08-27-c17-spec-ledger.md && git diff --cached --check && git commit -m "docs(C17): add contract-method breakdown"`：退出码 `0`；原始 stdout：`[cards/C17-charter-2 5571343] docs(C17): add contract-method breakdown`，3 个文件、292 行新增/修改；法定拆解稿已入库。
- 执行 `git show --stat --oneline --decorate HEAD && git status --short --branch && git rev-parse HEAD`：退出码 `0`；原始 stdout：`5571343 (HEAD -> cards/C17-charter-2) docs(C17): add contract-method breakdown`、`docs/ledgers/2026-08-27-c17-spec-ledger.md | 17 ++`、`docs/superpowers/specs/c17-breakdown.md | 274 +++++++++++++++++++++++++++++`、`docs/superpowers/specs/c17-contract.md | 1 +`、`3 files changed, 292 insertions(+)`、工作树状态 `## cards/C17-charter-2`、提交全 hash `5571343c06aa57075e24d4c06713c671bbc67be7`。
