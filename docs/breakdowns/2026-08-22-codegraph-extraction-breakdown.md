...
- **行为化验收**（机内）：①handoff 根 `go build ./... && go vet ./... && go test ./... -count=1` 全绿（含 TestRepoContractGate）；②`test ! -d internal/codegraph` 且 `grep -rn "Xsxdot/handoff/internal/codegraph" --include='*.go' .` 零命中；③go.mod 含 `charter/graph v0.1.0` 且 `grep -c '^replace' go.mod` = 0；④`handoff graph --help` 含 deprecated 字样，且 `handoff graph sym <符号> | python3 -m json.tool` 解析通过（运行时零 deprecation 污染）；⑤agentd codegraph 测试函数清单迁移前后一致（不许删测试收场）；⑥`handoff graph validate` 通过；`check` 判据**修订（2026-08-22 review 回写）**：原文「check 通过」写下时即已过期——B173 全量重扫已让 main 基线 check 长红（先于本卡），故判据改为「**fails 集合不增长**：钉死为收敛后的 16 条（4 new-direction + 12 over-budget，B173 尾部 target.json 重标定残余，挂 handoff B173 卡），新增任何一条即本卡缺陷」；target.json 不再含 `internal/codegraph/**`。①中 TestRepoContractGate 同理按此集合口径读（长红但集合钉死）。
