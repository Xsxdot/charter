...
13. **最优图 best.json（卡 `C1.8`，作废 C1.1 的 `TargetDomain.Paths` 原语）**：用户对「目标图」的定义更正——「目标域=一个包目录」是实现方引进的假设，用户从未提出；真正要的是**最优图 `best.json`**，「基于当下代码实现的功能，最优的子系统/领域结构应该是什么样」，`baseline.json` 的姊妹文件。**契约已冻结（2026-08-23，51 条，提交 `ccc61efa`）**：`docs/contracts/2026-08-23-codegraph-best-graph-contract.md`；spec 补写于其后。形态：best.json 接管整棵结构树（**子系统即顶层领域**，两套词汇合并成一棵树），target.json 瘦成契约面（`contracts` + `assembly`，`paths` 与 `TargetDomain` 删除），归属决议改走 容器→领域→子系统。四条 gap 判据（`container-misplaced` / `container-unplaced` / `domain-empty` 改判 / `best-dangling`）**全部 warn、不新增棘轮**——容器错位靠改标签即可消解，设 fail 等于给改标签发进度奖；不可伪造的进度条是边的合法性，沿用既有 `legacyBudget` 棘轮，`unplacedBudget` 删除。实测支撑：231 容器跨子系统 0、跨目录 0（容器是对的粒度），59/65 目录含多容器（容器比包细，能把一个包切进两个领域）。`migrate` 增一步从 v2 target 机械生成 best 初版，C1.6 是在初版上编辑而非手写 239 条。来源：2026-08-23 用户更正。
