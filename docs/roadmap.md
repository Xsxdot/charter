...
56. **C17 消费侧升 `graph/v0.10.0`**：canonical CLI 在 charter master 已挂 `flow`/`tree`；`handoff graph` 别名已于 B276 删除，不再重建。2026-08-28 从 master `a35b9524` 打 `graph/v0.10.0`（含 C17 控制流 CLI 与契约方法主语查看器）。handoff `go.mod` 仍钉 `v0.9.0` 时模块内 `go run … flow` 会 `unknown command`。消费侧升版见 handoff 卡 **B284**。与第 1i、第 6 条同族。来源：C17 plan §8.2-9、B277 finish、B276 删别名。
