...
- 2026-08-28：拆解核对澄清：§1.2 中“`graph/webui` 不在当前 charter 工作树”是起草时旧读数；当前工作树已跟踪 `graph/webui`，故本轮按既有 `deriveFlowPage` 缝地址纳入 charter 查看器实现范围。此澄清不改变 §2 的冻结 wire/语义，也不新增契约面；handoff 扫描配方仍为交棒欠账。
