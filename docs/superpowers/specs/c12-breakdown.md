...
> 状态：**已拍板（2026-08-26，协调者）**——P1～P5 五个岔口全部裁定，逐条理由见 §待拍板清单。扇出按 P1 裁决执行。
...
## 待拍板清单（已裁定 2026-08-26，协调者）

五条全部裁定，**四条采纳提案的推荐案，一条在推荐案基础上加了执行约束**。逐条理由如下。

### P1：子卡扇出粒度 —— **裁定 A（扇出六张），但串行链式执行，不并行**

**扇出。**核对了六张卡的有界文件集，实测**互不重叠**：K1 = Go 库 + `types.ts` + `besttree.ts` + 旧组件；
K2 = `scopepage.ts(+test)`；K3 = `flowpage.ts(+test)`；K4/K5 = 各自新建的组件文件；
K6 = `CodegraphPage.tsx` + 退役删除集。唯一的交集是 K1 改的三个旧组件被 K6 删除，
那是 DAG 顺序解决的先后关系，不是并行冲突。派卡资格四条成立，合并轮会白白浪费掉
拆解稿已经写全的四段式粒度。

**但不并行。**DAG 上真正能并行的只有两处（K2‖K3、K4‖K5），省下的墙钟有限；
代价是要把「合三条分支」的成本推给 integrate。**无人值守下，少一个失败模式比快两小时更值。**
所以六张卡**串行链式**跑：每张子卡的 `base_branch` 钉在前一张的工作分支上，
K6 的分支天然含全部产出，父卡 integrate 只做全量回归与接缝修补，不做三方合并。

### P2：同刀（responsibility 迁移）的时序 —— **裁定 A（K1 最前）**

采纳提案推荐。关键不在「哪边丢弃工少」——两案都有丢弃工，而在**丢弃工的风险不同**：

- A 的丢弃工是「给三个注定被删的旧组件穿一次线」，范围小、后果为零。
- B 的丢弃工是「K2～K5 五个新测试文件先按带 `responsibility` 的类型写夹具，K1 最后再全部换底」
  ——夹具换底是**跨五个文件的机械改动**，正是最容易漏一处而假绿的地方。

契约把这次删除定义为一次原子刀（§2.2-8/-10）。**先切，后面所有东西就都建在最终 wire 形状上。**

### P3：新两轴页面的文件落点 —— **裁定 A（新建页面组件，`CodegraphPage.tsx` 收敛为数据装配薄壳）**

三条理由：① spec 定的架构形态就是「纯函数派生层 + React 组件薄壳」，专门的页面组件正是这个形状，
B 的原地重写会让装配层和页面层继续糊在一个文件里；② A 的退役边界清晰——K6 的装配切换是一次
可回退的接线改动，B 是一个巨大的重写提交、review 面宽；③ 架构法第三条信号 1 已经命中过一次
（`Best*` 前缀家族 6 个文件 ≥5），新组件必须**按概念命名**（画布/右栏/流程图各归其名），
不能再长出第二个前缀家族——A 的分文件落点天然满足，B 把东西都塞回一个文件则回避了这个约束而非满足它。

### P4：`context.go` / `migrate.go` 的同刀处置 —— **裁定 A（`Summary` 改读 decls）**

架构法第五条：规则归数据所有者。走甲裁决的意图是**职责正文换一个唯一所有者**，
不是**让职责正文消失**。B 会让 agent 取数路径的领域摘要静默消失——那不是「未声明显形」，
是「有声明也看不到」，方向相反。

成本也不构成理由：拆解稿已核实 `AssembleContext` 持有 `repoRoot` 且 `:127` 已加载 decls，
把 decls 传进 `contextVocabulary` 即可。`migrate.go` 占位符改为不写该字段、`migrationNotes`
提示行同步删除。

### P5：冻结退场名单之外的旧视图文件去留 —— **裁定 A（无调用方即删，与装配切换同一提交）**

死代码留着必然在后续卡里被误当活路径引用——**C12 这张卡本身就是这么来的**
（C1.9/C1.10 对着一个没人审过的替身页面做形态基准，白费两轮）。这与 §2.5-37 对
`deriveDomainPage` 的处理是同一条逻辑：不留并存中间态。

保留项一处：`besttree.ts` 中被缝 1 复用的纯函数部分不删（拆解稿 §一 S2 边界澄清已圈出）。

### 拍板一并明确的两条执行约束

1. **子卡基线链式钉死**：K1 的 `base_branch` = 父卡基线；K2 = K1 的工作分支；依此类推到 K6。
   任一张卡返工时，其下游卡的基线要重新钉到返工后的分支上。
2. **handoff 侧三项交棒欠账不设 charter 子卡**（contract §6.2-1、拆解稿仓界声明）：
   responsibility 正文搬运、扫描配方自洽修复、宿主 `CodegraphFrame` 核验都不在本工作树，
   由父卡 finish 时统一移交，不混进本轮六张卡的验收。
...
1. `cd graph/webui && npm run typecheck && npm test` 全绿（~~预期 ≥21 文件~~ 计数判据作废，改行为判据：全绿 + 文件清单对账；实测 16 文件/203 支——见 §七 修订记录 R1）；`cd graph && go build ./... && go test ./... -count=1` 全绿（同刀回归口径，§4.3）。
...
## 七、修订记录（2026-08-27，finish 节点回写；源出 K6 审查移交，卡 C12 seq 5646）

K6 plan（`docs/superpowers/plans/c12.6-plan.md`）§9 跨卡审计留下三处「待拍板」，K6 审查轮已裁决、本节点追认回写：

- **R1｜K6 验收 1 的「预期 ≥21 文件」计数判据作废**：删除集大于新增集，文件数不增反降，计数前提不成立。替代判据为行为化的「全绿 + 文件清单对账」。实测终态：`npm test` 16 文件 / 203 支全绿（合并后 base 分支 2026-08-27 复跑同绿）。原条已在 §三-K6-③-1 就地划改。
- **R2｜D1 diff 视图不进两轴——追认**：选中非 baseline 视图时渲染基准快照 + 显式告知条「分支视图的两轴渲染未接入（二期）」。依据见 c12.6-plan §1.3 D1（缝 1 六字段冻结输入无 diff 维度，合成视图运行期即炸；系旧 banner 搁置决定的直系后代，非新回退）。
- **R3｜D2 工具条保留四件——追认**：视图下拉、失鲜徽标、单域提示 pill、刷新按钮（含 NOT_SCANNED 三分态）属数据层关注点，不在任何退役名单；单域提示文案随世界变化改为「重扫可获得两轴全景」。见 c12.6-plan §1.3 D2。
- **R4｜D3 P5-A 删除集扩大到 33 文件——追认**：判据不变（装配切换后生产 importer 归零即删），删除集按 c12.6-plan §0 枚举表扩大到 33 文件；besttree 因四个活消费方整文件保留。§三-K6 文件集清单（:415-420）为圈界示意，实际删除集以该枚举表为准。
