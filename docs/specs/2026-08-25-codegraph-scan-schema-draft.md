...
> **更正（2026-08-26，C12 contract 节点查证）**：消费点不止上面三处组件——`besttree.ts`
> 还有**六处模型透传**（`:11`、`:20`、`:83`、`:177`、`:201`、`:576`）。删字段的「同刀清单」
> 因此是 **3 处组件 + 6 处透传 + Go 侧 `BestDomain.Responsibility`（`best.go:35`）+
> `ValidateBest` 非空检查（`:92-94`）+ `best_test.go:55-57` 的序列化钉值**，共九处 TS
> 加三处 Go。完整清单以契约 §2.2-8/-10 为准。
>
> 另：`BestDomain.Responsibility` **无 `omitempty`** 且测试钉了空值序列化——删它必然改变
> wire 字节形状，这是「必须同一提交」的技术依据，不是流程洁癖。
...
### 8.2 容器只挂叶子领域 —— 已有闸，改为「保持项」

> **更正（2026-08-26，C12 contract 节点查证）**：本节初稿写的是「这条没人写过、没人查过，
> 今天成立是运气」——**错的**。我只查了数据（0 违例），没查有没有执法代码。
> 实际上 `graph/codegraph/best.go:127-135` 的 `ValidateBest` 早就在执法：
> `hasChild[domainID]` 命中即产出 issue「容器 %s 挂在非叶子领域 %s」，C1.8（`dc567ce`，
> 2026-08-24）落地，`graph/cli/cli.go:139/:260` 两处调用。下面是更正后的正文。

两轴导航的地基：**一个领域要么有子领域、要么有容器，不能两样都有**（否则「由什么组成」
这一格答不上来）。现状实测 **0 违例**（20 个叶子领域接住了全部 232 个容器），
**且这条已由 `ValidateBest` 执法**。

裁决：**保持项**——不新增检查，但不得删除或弱化 `best.go:127-135`。本卡的两轴导航把这条
从「碰巧成立的性质」升格为「地基」，所以它从此是一条有名字的不变式，改动它要当契约变更处理。
